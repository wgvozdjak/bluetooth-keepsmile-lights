...
function lightsMusicClick() {
  ...
  function updateColors() {
    ...
    const currentEnergy = Object.values(rangeEnergies).reduce(
      (a, b) => a + b,
      0
    );
    ...
    const energyVariance = Math.sqrt(
      energyHistory.reduce((a, b) => a + Math.pow(b - averageEnergy, 2), 0) /
    ...
    if (isBeat) {
      ...
      const normalizedEnergy =
        currentEnergy / (255 * Object.keys(FREQUENCY_RANGES).length);
